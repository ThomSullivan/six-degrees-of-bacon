...
def process_degree(info):
    ...
    for credit in current_cast:
        if credit[2] in existing_routes:
            ...
            cur1.execute('''INSERT OR IGNORE INTO ninth_degree (ID, previous, movie)
            VALUES ( ?, ?, ?)''', (credit[2], info[1], credit[1],))
            existing_routes.append(credit[2])
    conn1.commit()
BaconID = 4724
counter = 1
cur1.execute('''CREATE TABLE IF NOT EXISTS "ninth_degree" (
	"ID"	INTEGER UNIQUE,
	"previous"	INTEGER,
	"movie"	INTEGER,
	PRIMARY KEY("ID"),
	FOREIGN KEY("previous") REFERENCES "eighth_degree"("ID")
);''')
#get the list of tuples for use
cur1.execute('''SELECT ID FROM eighth_degree''')
