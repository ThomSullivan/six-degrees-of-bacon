...
def process_degree(info):
    ...
    for credit in current_cast:
        if credit[2] in existing_routes:
            ...
            cur1.execute('''INSERT OR IGNORE INTO tenth_degree (ID, previous, movie)
            VALUES ( ?, ?, ?)''', (credit[2], info[1], credit[1],))
            existing_routes.append(credit[2])
    conn1.commit()
BaconID = 4724
counter = 1
cur1.execute('''CREATE TABLE IF NOT EXISTS "tenth_degree" (
	"ID"	INTEGER UNIQUE,
	"previous"	INTEGER,
	"movie"	INTEGER,
	PRIMARY KEY("ID"),
	FOREIGN KEY("previous") REFERENCES "ninth_degree"("ID")
);''')
#get the list of tuples for use
cur1.execute('''SELECT ID FROM ninth_degree''')
...
tables = ['first_degree','second_degree','third_degree','fourth_degree','fifth_degree','sixth_degree',
            'seventh_degree','eighth_degree','ninth_degree']
existing_routes = []
existing_movies = []
for table in tables:
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for person in previous_degree:
            ...
            executor.map(process_degree, tup)
